...
The engine's on-disc catalogs (`X3.00` / `X3.10` / `X3.20`) store, per file,
an **explicit `[offset : 3 bytes LE sectors][size : 4 bytes LE]`** record —
verified from the decompiled reader `FUN_00187e28` and confirmed against the
disc (all Disc-1 movie offsets/sizes reproduce exactly; offsets are relative to
the catalog/mount base, a constant +26 sectors above the `Lba*.txt` offset,
which equals `X3.10`'s size). `repack.py` today does the simple thing:
...
**Growing a file is possible but not yet implemented here.** Because offsets
are stored (not accumulated), a bigger file does **not** require rebuilding the
container chain: append its new bytes to free space (e.g. after the last
container, which is the ISO tail) and rewrite just that entry's 3-byte sector
offset + 4-byte size in the catalog, then grow the last container's ISO9660
directory size + the PVD volume size. Neighbours are independent. The worked
example that needs this is HD FMV — see [FMV-HD-SCHEME.md](FMV-HD-SCHEME.md).
(Earlier notes here called offsets "implicit / full rebuild required"; that was
an artifact of `Lba*.txt` being regenerated by accumulation — corrected.)
